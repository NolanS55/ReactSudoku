                ...
                row.map((tile, index) => {
                    if((rowIndex + 1) % 3 === 0 && rowIndex < 6) {
                        if((index + 1) % 3  === 0 && index < 6) {
                            return <div className="sqaure" style={(index % 2 === 0) ? {backgroundColor : colorOne, borderStyle: 'solid', borderColor: 'black', borderBottomWidth : '2px', borderRightWidth : '2px'} : {backgroundColor: colorTwo, borderStyle: 'solid', borderColor: 'black', borderRightWidth : '2px', borderBottomWidth : '2px'}} key={index}>
                            <button className='tile' onClick={() => placeNumber(rowIndex, index, num)}>{tile}</button>
                            </div>
                        }
                        else {
                            return <div className="sqaure" style={(index % 2 === 0) ? {backgroundColor : colorOne, borderStyle: 'solid', borderColor: 'black', borderBottomWidth: '2px'} : {backgroundColor: colorTwo, borderStyle: 'solid', borderColor: 'black', borderBottomWidth: '2px'}} key={index}>
                            <button className='tile' onClick={() => placeNumber(rowIndex, index, num)}>{tile}</button>
                            </div>
                        }
                    }
                    else if((index + 1) % 3  === 0 && index < 6) {
                        return <div className="sqaure" style={(index % 2 === 0) ? {backgroundColor : colorOne, borderStyle: 'solid', borderColor: 'black', borderRightWidth : '2px'} : {backgroundColor: colorTwo, borderStyle: 'solid', borderColor: 'black', borderRightWidth : '2px'}} key={index}>
                        <button className='tile' onClick={() => placeNumber(rowIndex, index, num)}>{tile}</button>
                        </div>
                    }
                    else {
